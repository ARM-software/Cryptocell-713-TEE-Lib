                               ...
                               uint32_t  *pSignD)                   /*!< [in] Pointer to D-part of the signature (called also S-part). */
{
    ...
    err = PkaInitAndMutexLock(pDomain->ordSizeInBits , &pkaReqRegs);
    if (err != CC_OK) {
        return err;
    }
    ...
    PKA_MOD_INV_W_EXP(rKinv, rEphK, rT);
                                                                      ...
                                                                      CC_PKA_ECPKI_SCALAR_MUL_BUFF_MAX_LENGTH_IN_WORDS)*/
{
    ...
    while (1) {
        ...
        if(isEphemerKeyInternal) {
            ...
            /* set properly LE bytes order for pTempBuff, when BE PC is used */
            pEphemKeyBuf[ordSizeInWords-1] = 0; /*zero MSWord*/
            err = CC_RndGenerateVectorInRange(
                    f_rng, p_rng, pDomain->ordSizeInBits, (uint8_t*)pMaxVect/* maxVect*/, (uint8_t*)pEphemKeyBuf);
            if (err) {
                goto End;
            }
            /* set words Endianness for pEphKeyBuf, when BE PC is used  */
        }
