...
CCError_t RndGenerateWordsArrayInRange(CCRndGenerateVectWorkFunc_t f_rng,
                                              void *p_rng,
 ...
 * @param[in]  p_rng              - Pointer to the random context.
                                        ...
                                        uint32_t         *TempBuff3_ptr )      /*in*/
{
        ...
        *  then return error. This checking is for preventing KW warnings   */
        ...
        /* seed size must allow adding counters for hashing without overflow of temp buffers:*/
        ...
        while (isPrime != CC_TRUE) {
                ...
                if (generateSeed == 1) {
                        ...
                        while (isSeedValid == 0) {
                                Error = f_rng(p_rng, (unsigned char *)S_ptr, (size_t)seedSizeBytes);
        ...
        while (isPrime != CC_TRUE) {
                ...
                *   multiple of 32 bits 				      */
                ...
                if (cmpRes == CC_COMMON_CmpCounter1GreaterThenCounter2) {
                        Error = RsaPrimeTestCall(f_rng, p_rng,
                                                          P_ptr,
                                                          modSizeWords,
                                                          countMilRabTests,
                                                          (int8_t*)&isPrime,
                ...
                /* defines: 4096 -> PGEN_COUNTER_MAX_VAL,  L = 1024 -> PRIME_MOD_MIN_VAL */
 ...
 *
 */
static CCError_t DhX942CreateGenerator(
                                              ...
                                              CCDhExpTemp_t  *expTempBuff_ptr)     /*in */
{
        ...
        // J-factor is used in other functions
        /* The return error identifier */
        ...
        Error = RndGenerateWordsArrayInRange(f_rng, p_rng,
        ...
        /* set 1 to tempBuff1_ptr for comparing */
                                           ...
                                           CCDhKgData_t  *DHKGbuff_ptr              /*in */)
{
        ...
        Error = DhX942FindPrimeQ(f_rng, p_rng,
                                      orderQsizeBits,            /*in */
                                      seedSizeBits,              /*in */
                                      generateSeed,              /*in */
                                      Q_ptr,                     /*out*/
        ...
        Error = DhX942FindPrimeP(f_rng, p_rng,
                                      modPsizeBits,        /*in */
                                      orderQsizeBits,      /*in */
                                      seedSizeBits,        /*in */
                                      P_ptr,               /*out*/
        ...
        if (generatorG_ptr != NULL) {
                Error = DhX942CreateGenerator(f_rng, p_rng,
                                                   P_ptr,                                       /*in */
                                                   modPsizeBits,                                /*in */
                                                   Q_ptr,                                       /*in */
                                                   orderQsizeBits,                              /*in */
        ...
        /* Use J-factor from previous function */
        if (factorJ_ptr != NULL) {
                PkiLongNumDiv(
                                    P_ptr,                 /*numerator B*/
...
*
* @param [in]  f_rng              - Pointer to DRBG function
* @param [in]  p_rng        	  - Pointer to the random context - the input to f_rng.
