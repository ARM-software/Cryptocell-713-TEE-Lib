...
static CCKdfDerivFuncMode_t MakeKDFDeriveFuncMode(CCDhDerivationFuncMode_t deriveFunc)
{
        ...
        /* to avoid the problem of two same definitions in CCDhDerivationFuncMode_t */
        if (deriveFunc == CC_DH_Concat_Der_mode)
                return CC_KDF_ConcatDerivMode;
        switch (deriveFunc) {
        case CC_DH_ASN1_Der_mode:
                outDeriveFunc = CC_KDF_ASN1_DerivMode;
                break;
        /* returning the sa,e deriveFunc for CC_DH_X963_DerMode and CC_DH_Concat_Der_mode */
        ...
        case CC_DH_PKCS3_mode:
            ...
            PKCS#3:  set x private random value according to following:
                ...
                if (L == 0) {
                        ...
                        Error = CC_RndGenerateVectorInRange( f_rng, p_rng,
                        ...
                        *ClientPrvKeySize_ptr = CALC_FULL_BYTES(L);
                        ...
                        /* if L < modulus size, then generate random x of size L bits */
                                ...
                                /* random generation */
                                Error = f_rng((void *)p_rng, (unsigned char *)ClientPrvKey_ptr, *ClientPrvKeySize_ptr );
