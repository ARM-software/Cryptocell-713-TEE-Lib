                               ...
                               uint32_t *pCertBufferWordSize)
{
        ...
        if ((pCertHeader->certSize + BSV_CERT_RSA_KEY_SIZE_IN_WORDS) < pCertHeader->certSize){
        ...
        if ((pCertHeader->certSize + BSV_CERT_RSA_KEY_SIZE_IN_WORDS) > *pCertBufferWordSize) {
                CC_PAL_LOG_ERR("Certificate size too big\n");
                return CC_BOOT_IMG_VERIFIER_INV_INPUT_PARAM;
        }
        ...
        error = flashRead_func(certAddress + sizeof(CCSbCertHeader_t),
                               ...
                               ((pCertHeader->certSize + BSV_CERT_RSA_KEY_SIZE_IN_WORDS) * CC_32BIT_WORD_SIZE) - sizeof(CCSbCertHeader_t),
                               userContext);
        if (error != CC_OK) {
        ...
        *pCertBufferWordSize = (pCertHeader->certSize + BSV_CERT_RSA_KEY_SIZE_IN_WORDS);
