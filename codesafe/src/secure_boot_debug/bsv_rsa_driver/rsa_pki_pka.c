...
static uint32_t isPkaFunctional(unsigned long hwBaseAddress)
{
    uint32_t isFatalError = 0;

    /* when fatal error is asserted PKA is not functional */
    CC_BSV_IS_FATAL_ERR_ON(hwBaseAddress, isFatalError);
    if (isFatalError == 1) {
        return CC_BSV_FATAL_ERR_IS_LOCKED_ERR;
    }
			      ...
			      unsigned long  virtualHwBaseAddr)
{
	...
	/* remaining entries set to PKA_SIZE_ENTRY_NOT_USED for debugging goals - except the last one (LenIDmax) */
	for (i = 2; i < (RSA_PKA_MAX_COUNT_OF_REGS_SIZES-1); i++) {
		SB_HAL_WRITE_REGISTER(SB_REG_ADDR(virtualHwBaseAddr, PKA_L0) + 4*i, PKA_SIZE_ENTRY_NOT_USED);
	}
	/*  Set LenIDmax operation size*/
	SB_HAL_WRITE_REGISTER(SB_REG_ADDR(virtualHwBaseAddr, PKA_L0) + 4*(LenIDmax), regSizeInPkaWords*RSA_PKA_BIG_WORD_SIZE_IN_BITS);
 ...
 *                       CC_BSV_FATAL_ERR_IS_LOCKED_ERR
  *
 */
uint32_t RSA_PKA_InitPka( uint32_t   opSizeInBits,	        /*in - modulus size in bits*/
		      ...
		      unsigned long  virtualHwBaseAddr) /*in*/
{
	...
	rc = isPkaFunctional(virtualHwBaseAddr);
	if (rc != CC_OK) {
	    return rc;
	}
 ...
 */
 void RSA_HW_PKI_PKA_FinishPKA( unsigned long virtualHwBaseAddr /*in*/ )
 {
    int32_t i;
    uint32_t size,  addr;
    int32_t countOfRegs = PKA_MAX_COUNT_OF_PHYS_MEM_REGS;

    /* FUNCTION LOGIC */
    /* clear used PKA registers for security goals */
    /* calculate size of register in words */
    RSA_PKA_ReadRegSize(size, LEN_ID_MAX_BITS, virtualHwBaseAddr);
    size = CALC_FULL_32BIT_WORDS(size);

    /* clear ordinary and temp registers without PKA operations */
    for (i = 0; i < countOfRegs; i++) {
        addr = RSA_PKA_GetRegAddress(i/*VirtReg*/,virtualHwBaseAddr/*physAddr*/);
        RSA_HW_PKI_HW_CLEAR_PKA_MEM(virtualHwBaseAddr, addr, size);
    }
    addr = RSA_PKA_GetRegAddress(PKA_T1/*VirtReg*/,virtualHwBaseAddr/*physAddr*/);
    RSA_HW_PKI_HW_CLEAR_PKA_MEM(virtualHwBaseAddr, addr, size);
    addr = RSA_PKA_GetRegAddress(PKA_T0/*VirtReg*/,virtualHwBaseAddr/*physAddr*/);
    RSA_HW_PKI_HW_CLEAR_PKA_MEM(virtualHwBaseAddr, addr, size);
