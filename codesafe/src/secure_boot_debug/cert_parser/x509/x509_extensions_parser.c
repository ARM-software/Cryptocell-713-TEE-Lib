...
/*@
    requires \valid(*ppCert + (0 .. (((uint8_t*)endAddress - (uint8_t*)*ppCert) - 1)));
    ensures ((\result == 0) && \valid(*ppCertPropHeader)) ||
            (\result != 0);
 */
					...
					unsigned long   endAddress)
{
	...
	rc = UTIL_Asn1ReadItemVerifyTagFW(ppCert, &asn1Data, CC_X509_CERT_CTX_EXT_TAG_ID, endAddress);
	if (rc != CC_OK) {
		 CC_PAL_LOG_ERR("1. Failed to read CC_X509_CERT_CTX_EXT_TAG_ID \n");
		return CC_SB_X509_CERT_PARSE_ILLEGAL_VAL;
	}
	/* read SEQ */
	rc = UTIL_Asn1ReadItemVerifyTagFW(ppCert, &asn1Data, CC_X509_CERT_SEQ_TAG_ID, endAddress);
	if (rc != CC_OK) {
	...
	for (extNum=0; extNum < CC3X_X509_CERT_EXT_NUMBER && nextBuffOffset < certSize; extNum++){
		...
		/* read OBJ ID */
		rc = UTIL_Asn1ReadItemVerifyTagFW(ppCert, &asn1Data, CC_X509_CERT_OBJ_IDENTIFIER_TAG_ID, endAddress);
		if (rc != CC_OK) {
			 CC_PAL_LOG_ERR("3. Failed to read CC_X509_CERT_SEQ_TAG_ID\n");
			return CC_SB_X509_CERT_PARSE_ILLEGAL_VAL;
		}
		...
		UTIL_ASN1_GET_NEXT_ITEM_RET(*ppCert, asn1Data.itemSize, endAddress);

		rc = UTIL_Asn1ReadItemVerifyTagFW(ppCert, &asn1Data, CC_X509_CERT_BOOL_TAG_ID, endAddress);
		if (rc != CC_OK) {
		...
		rc = UTIL_Asn1ReadItemVerifyTagFW(ppCert, &asn1Data, CC_X509_CERT_OCT_STR_TAG_ID, endAddress);
		if (rc != CC_OK) {
		...
		if ((asn1Data.itemSize > extBuffSize) && (**ppCert == 0)) {
			UTIL_ASN1_GET_NEXT_ITEM_RET(*ppCert, 1, endAddress);
			asn1Data.itemSize--;
