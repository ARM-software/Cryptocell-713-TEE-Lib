...
int32_t Sign_v15(RSA  *pRsaPrivKey,
    ...
    uint32_t SignatureSize  = BSV_CERT_RSA_KEY_SIZE_IN_BYTES;
...
int32_t Sign_v21(RSA   *pRsaPrivKey,
    ...
    EVP_MD_CTX md_ctx;
    uint8_t EM[BSV_CERT_RSA_KEY_SIZE_IN_BYTES] = { 0 };
    uint8_t pDecrypted[BSV_CERT_RSA_KEY_SIZE_IN_BYTES] = { 0 };
    int32_t status = -1;
    ...
    if (RSA_private_encrypt(BSV_CERT_RSA_KEY_SIZE_IN_BYTES, EM, pSignature, pRsaPrivKey, RSA_NO_PADDING) == -1) {
        return (status);
    }

    /* verify the data */
    if (RSA_public_decrypt(BSV_CERT_RSA_KEY_SIZE_IN_BYTES, pSignature, pDecrypted, pRsaPrivKey, RSA_NO_PADDING) == -1) {
        return (status);
    }
...
int32_t Verify_v21(RSA *pRsaPubKey,
    ...
    if (RSA_public_decrypt(BSV_CERT_RSA_KEY_SIZE_IN_BYTES, pSignature, pDecrypted, pRsaPubKey, RSA_NO_PADDING) == -1) {
        return (status);
    }
...
int32_t CC_CommonRsaVerify(uint32_t  pkcsVersion,
    ...
    pRsaPubKey->n = BN_bin2bn(pPubKey, BSV_CERT_RSA_KEY_SIZE_IN_BYTES, NULL);
...
int32_t CC_CommonRsaSign(uint32_t pkcsVersion,
    ...
    if ((NULL != pPrivKeyPwdFileName)) {
        if (CC_CommonGetPassphrase(pPrivKeyPwdFileName, &pwd)) {
...
int32_t CC_CommonRsaEncrypt(uint8_t *pPubKey,
    ...
    if (status != BSV_CERT_RSA_KEY_SIZE_IN_BYTES) { /* expected encryped size is RSA modulus sieze */
...
int32_t CC_CommonRsaDecrypt(char *pPrivKeyFileName,
    ...
    if (status != (-1)) {  /* RSA_private_decrypt returns the size of the recovered plaintext. On error, -1 is returned */
        UTIL_LOG_ERR("RSA_private_decrypt is OK %d\n", status);
        status = 0;
    }
...
int32_t CC_CommonRsaCalculateH(const uint8_t *N_ptr, uint8_t *H_ptr) {
    int8_t *H_res = NULL, *N_Temp = NULL;
    char *H_resTemp = NULL;
    int32_t len, i;
    int32_t status = -1;
    uint32_t s = BSV_CERT_RSA_KEY_SIZE_IN_BITS + 2;
    ...
    N_Temp = (int8_t *)malloc((BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2) * sizeof(int8_t));
    if (NULL == N_Temp) {
    ...
    H_res = (int8_t *)malloc((BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2) * sizeof(int8_t));
    if (NULL == H_res) {
    ...
    if (len < BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2) {
        ...
        for (i = 0; i < (int32_t)(BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 - len); i++) {
            H_res[i] = '0';
        }
    }

    /* Set the output with 0 and than copy the result */
    memset(H_ptr, 0, (BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2)); //VERIFY THAT USER SENDS THE SAME SIZE
    memcpy((int8_t *)H_ptr, (int8_t *)H_res, BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2);
 ...
 */
/*********************************************************/
int32_t CC_CommonRsaLoadKey(char *PemEncryptedFileName_ptr, uint8_t *Key_ptr, uint8_t *PemDecryted) {
    RSA *rsa_pkey = NULL;
    char buffer[256];
    BIO *out = NULL;
    int32_t status = -1;
    ...
    *PemDecryted = '\0';
    ...
    while (BIO_gets(out, buffer, sizeof(buffer)) > 0) {
        if (strlen((char *)PemDecryted) + strlen(buffer) + 2 > RSA_PRIVATE_KEY_SIZE) {
            UTIL_LOG_ERR("Internal error: The output buffer is too few\n");
            break;
        }
        strncat((char *)PemDecryted, buffer, strlen(buffer));
    }
    status = 0;
