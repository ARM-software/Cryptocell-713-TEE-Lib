 ...
 */
int32_t CC_CommonUtilCopyDataFromRawTextFile (char *fileName, uint8_t *outBuff, uint32_t *outBuffLen)
{
	int32_t status = 0;
	FILE *fd;
	long int actualFileLen=0;
	size_t actualRead=0;
    ...
    maxBytesToRead = (actualFileLen > (int32_t) ((*outBuffLen)) ? (size_t)(*outBuffLen) : (size_t)actualFileLen);
	if (0 == maxBytesToRead) {
 ...
 */
int32_t CC_CommonUtilCopyDataFromTextFile (char *fileName, uint8_t *outBuff, uint32_t *outBuffLen)
{
	...
	/* calculate max bytes to read. should be the min of bytes in file and buffer size*/
    maxBytesToRead = (actualFileLen > (int32_t)(*outBuffLen * 5)) ? (size_t)(*outBuffLen * 5) : (size_t)actualFileLen;
	if (0 == maxBytesToRead) {
		UTIL_LOG_ERR( "ilegal maxBytesToRead == 0\n");
		status = 1;
		goto EXIT;
	...
	/* read file content */
	actualBytesRead = fread(filebufptr, 1, maxBytesToRead, fd);
	j=0;
	k=0;
			...
			if ((filebufptr[i] == ' ') ||
				...
				if (k>0) {
					...
					if ((LONG_MIN == tempNum) ||
						...
						status = 1;
						goto EXIT_AND_FREE;
					} else if (tempNum > 0xff) {
						UTIL_LOG_ERR( "strtol failed. tempNum > 0xff\n");
						status = 1;
						goto EXIT_AND_FREE;
					}
					/* cast back to byte */
					outBuff[j++] = (uint8_t)tempNum;
					k = 0;
				}
				continue;
			} else {
 ...
 */
int32_t CC_CommonUtilCopyDataFromBinFile(char *fileName, uint8_t *outBuff, uint32_t *outBuffLen)
{
	int32_t rc = 0;
	FILE *fd;
	size_t actualRead = 0;
 ...
 */
int32_t CC_CommonUtilCopyBuffToBinFile(char *fileName, uint8_t *inBuff, uint32_t inBuffLen)
{
	int32_t rc = 0;
	...
	if (actualWriten != inBuffLen) {
		UTIL_LOG_ERR( "failed to write data to file actual written %zu, expected %d\n", actualWriten, inBuffLen);
		rc = 1;
	}
	...
	UTIL_LOG_ERR( "%zu bytes were written %s\n", actualWriten, fileName);
 ...
 */
/*********************************************************/
int32_t CC_CommonGetPassphrase(char *pPwdFileName, uint8_t **pwd)
{
