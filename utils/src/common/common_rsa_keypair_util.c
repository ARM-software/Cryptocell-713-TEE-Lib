...
DxRsaKeyNandNp_t gNAndNp = { { 0 }, { 0 } };
uint8_t gN[BSV_CERT_RSA_KEY_SIZE_IN_BYTES +1] = {0};
...
*/
/*********************************************************/
int32_t CC_CommonGetNbuffFromKeyPair(char *PemEncryptedFileName_ptr, char *pwdFileName, uint8_t *pNbuff, uint32_t *pNbuffSize)
{
	...
	if (*pNbuffSize != BSV_CERT_RSA_KEY_SIZE_IN_BYTES) {
		return status;
	}
	...
	UTIL_LOG_BYTE_BUFF("gN", gN, BSV_CERT_RSA_KEY_SIZE_IN_BYTES);

	/* copy the Np to the end of N */
	memcpy(pNbuff, gN, BSV_CERT_RSA_KEY_SIZE_IN_BYTES);
	*pNbuffSize = BSV_CERT_RSA_KEY_SIZE_IN_BYTES;
	status = 0;
...
*/
/*********************************************************/
int32_t CC_CommonGetNAndNpFromKeyPair(char *PemEncryptedFileName_ptr,
                                      char *pwdFileName,
                                      uint8_t *pNAndNp,
                                      uint32_t *pNAndNpSize)
{
	...
	/* copy the Np to the end of N */
	memcpy(pNandNpBuff->pNBuff, gN, BSV_CERT_RSA_KEY_SIZE_IN_BYTES);
	memcpy(pNandNpBuff->pNpBuff, gNp, NP_SIZE_IN_BYTES);
	*pNAndNpSize = (BSV_CERT_RSA_KEY_SIZE_IN_BYTES+NP_SIZE_IN_BYTES);
	UTIL_LOG_BYTE_BUFF("gN", gN, BSV_CERT_RSA_KEY_SIZE_IN_BYTES);
...
*/
/*********************************************************/
int32_t CC_CommonGetNAndNpFromPubKey(char *pubKeyFileName_ptr, uint8_t *pNAndNp, uint32_t *pNAndNpSize)
{
	int32_t status = -1;
	RSA *rsa_pkey = NULL;
	DxRsaKeyNandNp_t *pNandNpBuff = (DxRsaKeyNandNp_t *)pNAndNp;
...
int32_t CC_CommonCalcHBKFromBuff(int8_t* pNBuff, uint8_t *pHash, int32_t hashSize)
{
    ...
    UTIL_LOG_BYTE_BUFF("gN", (uint8_t *)&gNAndNp, BSV_CERT_RSA_KEY_SIZE_IN_BYTES);

    /* calculate the Np */
    bn_n = BN_bin2bn((unsigned char *)pNBuff, BSV_CERT_RSA_KEY_SIZE_IN_BYTES, bn_n);
    if (NULL == bn_n) {
...
*/
/*********************************************************/
int32_t CC_CommonCalcHBKFromFile(char* pubKeyFileName_ptr, uint8_t *pHash, int32_t hashSize)
{
	...
	/* copy the Np to the end of N and calc hash on both */
	memcpy(gNAndNp.pNBuff, gN, BSV_CERT_RSA_KEY_SIZE_IN_BYTES);
	memcpy(gNAndNp.pNpBuff, gNp, NP_SIZE_IN_BYTES);
	/* write hash*/

	/* calculate hash and write to */
	if (CC_CommonCalcHash((uint8_t *)&gNAndNp, BSV_CERT_RSA_KEY_SIZE_IN_BYTES+NP_SIZE_IN_BYTES, pHash, hashSize) != 0) {
		UTIL_LOG_ERR ("Common_CalcHashOnPubKey failed\n");
		goto END;
	}
