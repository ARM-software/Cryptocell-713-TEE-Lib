 ...
 */
/*********************************************************/
int32_t CC_CommonGetKeyPair(RSA **pRsaKeyPair, char *PemEncryptedFileName_ptr, uint8_t *Key_ptr)
{
	FILE *fp = NULL;
 ...
 */
/*********************************************************/
int32_t CC_CommonGetPubKey(RSA **pRsaKeyPair, char *PemEncryptedFileName_ptr)
{
	FILE *fp = NULL;
...
*/
/*********************************************************/
SBUEXPORT_C int32_t CC_CommonRsaCalculateNp(const uint8_t *N_ptr,
					  uint8_t *NP_ptr)
{
    char *N_Temp = NULL;
	int32_t  status  = -1;
	...
	N_Temp= (char *)malloc ((BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2) * sizeof (int8_t));
	if (NULL == N_Temp) {
					     ...
					     NP_RESULT_TYPE_t resultType)
{
	...
	NP_res = malloc(NP_SIZE_IN_BYTES);
	if (NP_res == NULL) {
		UTIL_LOG_ERR("failed to malloc.\n");
		goto calcNpInt_end;
	}
	...
	if (resultType == NP_BIN) {
		...
		memcpy ((uint8_t *)(NP_ptr + (NP_SIZE_IN_BYTES - len)), NP_res, len);
	} else { /* resultType == HEX*/
		...
		memcpy ((int8_t *)(NP_ptr + (NP_SIZE_IN_BYTES * 2 + 2 - len)), NP_res, len);
	}
