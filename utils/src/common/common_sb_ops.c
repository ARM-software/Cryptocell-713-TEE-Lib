             ...
             uint8_t *Key_ptr)
{
	...
	int SignatureSize  = BSV_CERT_RSA_KEY_SIZE_IN_BYTES;
	...
	if (!EVP_SignFinal(md_ctx, Signature_ptr, (uint32_t *)&SignatureSize, pKey))
 ...
 */
 /*********************************************************/
int Sign_v21(RSA *pRsaPrivKey, uint8_t *DataIn_ptr, int DataInSize, uint8_t *Signature_ptr)
{
    unsigned char pDigest[HASH_SHA256_DIGEST_SIZE_IN_BYTES];
    unsigned int uDigestLen = HASH_SHA256_DIGEST_SIZE_IN_BYTES;
    EVP_MD_CTX md_ctx;
    unsigned char EM[BSV_CERT_RSA_KEY_SIZE_IN_BYTES];
    unsigned char pDecrypted[BSV_CERT_RSA_KEY_SIZE_IN_BYTES];
    int status = -1;

    if ((pRsaPrivKey == NULL) || (DataIn_ptr == NULL) || (Signature_ptr == NULL)) {
    ...
    if (RSA_private_encrypt(BSV_CERT_RSA_KEY_SIZE_IN_BYTES, EM, Signature_ptr, pRsaPrivKey,
    RSA_NO_PADDING) == -1)
        SIGN21_RELEASE("RSA_private_encrypt")

    /* verify the data */
    if (RSA_public_decrypt(BSV_CERT_RSA_KEY_SIZE_IN_BYTES, Signature_ptr, pDecrypted, pRsaPrivKey,
    RSA_NO_PADDING) == -1)
...
*/
/*********************************************************/
SBUEXPORT_C int SBU_GetNFromKeyPairAndCalcH(char* PemEncryptedFileName_ptr, char *pwdFileName, uint8_t *N_ptr, uint8_t *H_ptr)
{
	char *H_res = NULL, *N_Temp = NULL, *H_resTemp = NULL;
	int len, i;
	int status = -1;
	unsigned long s = BSV_CERT_RSA_KEY_SIZE_IN_BITS + 2;
	int rc = 0;
	int nSize = BSV_CERT_RSA_KEY_SIZE_IN_BYTES;
	char *pwdPtr = NULL;
	...
	/* get N  buffer */
    rc = CC_CommonGetNbuffFromKeyPair(PemEncryptedFileName_ptr,
                                      pwdPtr,
                                      N_ptr,
                                      (uint32_t *)&nSize);
	EVP_cleanup();
	...
	N_Temp= (char *)malloc ((BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2) * sizeof (char));
	...
	memset (N_Temp, 0, (BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2));

	/* Copy the N to temp N */
	memcpy (N_Temp, N_ptr, BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2);

	/* Allocate the output buffer */
	if (NULL == (H_res = (char *)malloc ((BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2) * sizeof (char))))
		CALCULATE_H_RELEASE("Error during memory allocation.")

	BN_set_word (bn_two, 2);
	BN_set_word (bn_twos, 2 * s);

	bn_n = BN_bin2bn(N_ptr, BSV_CERT_RSA_KEY_SIZE_IN_BYTES, bn_n);
	if (NULL == bn_n)
	...
	if (len < BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2)
	{
		memmove (H_res + (BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 - len), H_res, len + 1);
		for (i = 0; i < (int)(BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 - len); i++)
			H_res[i] = '0';
	}

	/* Set the output with 0 and than copy the result */
	memset (H_ptr, 0, (BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2));
	memcpy (H_ptr, (char *)H_res, BSV_CERT_RSA_KEY_SIZE_IN_BYTES * 2 + 2);
...
*/
/*********************************************************/
SBUEXPORT_C int SBU_RAND_Bytes(int numBytes, uint8_t *buf)
{
	int result = -1;
...
*/
/*********************************************************/
SBUEXPORT_C int SBU_GetNAndNpFromKeyPair(char* PemEncryptedFileName_ptr,
                                         char *pwdFileName,
                                         uint8_t *PemDecryted)
{
    int rc = 0;
	int nAndNpSize = BSV_CERT_RSA_KEY_SIZE_IN_BYTES+NP_SIZE_IN_BYTES;
	char *pwdPtr = NULL;
	...
	/* get N and Np buffer */
    rc = CC_CommonGetNAndNpFromKeyPair(PemEncryptedFileName_ptr,
                                       pwdPtr,
                                       PemDecryted,
                                       (uint32_t *)&nAndNpSize);
    if (rc != 0) {
...
*/
/*********************************************************/
SBUEXPORT_C int SBU_GetNAndNpFromPubKey(char* PemEncryptedFileName_ptr, uint8_t *PemDecryted)
{
	int rc = 0;
	int nAndNpSize = BSV_CERT_RSA_KEY_SIZE_IN_BYTES+NP_SIZE_IN_BYTES;
	...
	/* get N and Np buffer */
    rc = CC_CommonGetNAndNpFromPubKey(PemEncryptedFileName_ptr,
                                      PemDecryted,
                                      (uint32_t*) &nAndNpSize);
	if (rc != 0) {
...
*/
/*********************************************************/
SBUEXPORT_C int SBU_GetHashOfNAndNpFromPubKey(char* pPemFileName_ptr, uint8_t *pHash, int hashSize)
{
	int rc = 0;
