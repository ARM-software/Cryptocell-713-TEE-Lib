                                        ...
                                        int8_t cryptoType)
{
    ...
    if ((actualFileLen == -1) || (actualFileLen > INT_MAX)){
        printf("ftell failed\n");
        goto END;
    }
    fseek(fd, 0, SEEK_SET);
...
SBUEXPORT_C uint32_t SBU_x509_AddPubKeyNpBuffExtension(int8_t *pKeyPairFileName, int8_t *pwdFileName)
{
    ...
    /* calculate N and Np */
    rc =  CC_CommonGetNAndNpFromKeyPair(pKeyPairFileName, pwdPtr, (uint8_t *)&pNAndNp, &pNAndNpSize);
    if (rc != 0) {
        UTIL_LOG_ERR("failed to CC_CommonGetNAndNpFromKeyPair %s pwd %s\n", pKeyPairFileName, pwdPtr);
        goto END;
    }
                                                      ...
                                                      uint8_t *pPubKeyFileName)
{
    ...
    keyCertBody.nvCounter = nvCounter;
 ...
 *
 * @param[in] nvCounter - revocation version of the certificate, must be at least as defined in OTP
...
{
    uint32_t rc = 0;
    uint32_t certBody[CC_SB_MAX_CONTENT_CERT_BODY_SIZE_IN_BYTES/sizeof(uint32_t)];
    ...
    /* Note! it is expected that the data is always 32 bit dividable */
    certBody[i] = nvCounter;
    i += 1;
    memcpy((uint8_t *)&certBody[i] ,pNonce, sizeof(CCSbNonce_t));
    i += sizeof(CCSbNonce_t)/sizeof(uint32_t);
    memcpy((uint8_t *)&certBody[i], pImageList, SW_REC_SIGNED_DATA_SIZE_IN_BYTES*numOfImages);
    certBodySize = sizeof(uint32_t) + sizeof(CCSbNonce_t) + SW_REC_SIGNED_DATA_SIZE_IN_BYTES*numOfImages;
