...
def CreateCertUtility(sysArgsList):
    try:
        ...
        data_dict, config = enabler_cert_config_file_parser(config_fname, log_file, ProjDict)
        if data_dict == None:
        ...
        propHeader = build_certificate_header(log_file, DEBUG_ENABLER_TOKEN, ProjDict, LIST_OF_CONF_PARAMS, data_dict['rma_mode'], data_dict['hbk_id'], data_dict['lcs'], data_dict['add_data'])
        if propHeader == None:
            print_and_log(log_file, "**** build_certificate_header failed ****\n")
            exit_main_func(log_file, config_fname, result)
