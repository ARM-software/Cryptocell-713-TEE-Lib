...
def CreateCertUtility(sysArgsList):
    try:
        ...
        data_dict, config = developer_cert_config_file_parser(config_fname, log_file, ProjDict)
        if data_dict == None:
        ...
        propHeader = build_certificate_header(log_file, DEBUG_DEVELOPER_TOKEN, ProjDict, LIST_OF_CONF_PARAMS, 0, 0, 0, data_dict['add_data'])
        if propHeader == None:
