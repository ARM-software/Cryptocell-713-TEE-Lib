...
def CreateCertUtility(sysArgsList):
    try:
        ...
        data_dict, config = content_cert_config_file_parser(config_fname, log_file, ProjDict)
        if data_dict == None:
        ...
        RecList = ImageFileAnalyzer(log_file, data_dict['images_table'], data_dict['load_verify_scheme'], data_dict['aes_ce_id'], DLLHandle, data_dict['aes_enc_key'], keyNonce.randStr, data_dict['crypto_type'], ProjDict)

        print_and_log(log_file, "**** calling SBU_x509_AddPropHeaderExtension **** \n")
        propHeader = CertHeader(log_file, LIST_OF_CONF_PARAMS, data_dict['hbk_id'], CERT_TYPE_CONTENT, data_dict['aes_ce_id'], data_dict['load_verify_scheme'], data_dict['crypto_type'], len(RecList), ProjDict, data_dict['add_data'])
        ...
        result = DLLHandle.SBU_x509_AddContentCertBodyExtension(data_dict['nvcounter_word'], nonceStr.encode('iso-8859-1'), BinStr.encode('iso-8859-1'), len(RecList))
        if result != 0:
