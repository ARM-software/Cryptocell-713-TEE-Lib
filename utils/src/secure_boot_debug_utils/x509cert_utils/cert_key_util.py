...
def CreateCertUtility(sysArgsList):
    try:
        ...
        data_dict, config = key_cert_config_file_parser(config_fname, log_file, ProjDict)
        if data_dict == None:
        ...
        propHeader = CertHeader(log_file, LIST_OF_CONF_PARAMS, data_dict['hbk_id'], CERT_TYPE_KEY, 0, 0, 0, 0, ProjDict, data_dict['add_data'])
        ...
        result = DLLHandle.SBU_x509_AddKeyCertBodyExtension(data_dict['nvcounter_word'], str.encode(data_dict["next_cert_pubkey"]))
        if result != 0:
