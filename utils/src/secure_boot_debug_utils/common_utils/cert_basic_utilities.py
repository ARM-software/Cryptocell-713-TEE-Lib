...
PROJ_EXT_CONFIG = CURRENT_PATH + path_div + ".." + path_div + ".." + path_div + 'proj.ext.cfg'
...
def parseConfFile(listOfDefines):
    cfgFileList = [PROJ_EXT_CONFIG, PROJ_CONFIG]
    ProjDict = dict()
    ProjDict['is_add_data'] = int(0)
    ProjDict['is_iot_cert'] = int(0)
    ProjDict['majorKey'] = int(0)
    ProjDict['minorKey'] = int(0)
            ...
            for line in lines:
                for define in listOfDefines:
                    if re.match(define,line):
                        tmpList=line.split("=")
                        defValue = int(tmpList[len(tmpList)-1],16)
                        if str(define) == "CONFIG_SB_SUPPORT_IOT":
                            ProjDict['is_iot_cert'] = defValue
                        if str(define) == "CERT_VERSION_MAJOR":
                            ProjDict['majorKey'] = defValue
                        if str(define) == "CERT_VERSION_MINOR":
                            ProjDict['minorKey'] = defValue
                        if str(define) == "CC_CONFIG_BSV_CERT_WITH_USER_ADDITIONAL_DATA":
                            ProjDict['is_add_data'] = defValue

            fob.close()
    except IOError as Error1:
        (errno, strerror) = Error1.args
        print("\n Error in openning file - %s" %file)
        sys.exit(1)

    if ProjDict['is_iot_cert'] == 1:
        version = CC3X_SB_CERT_VERSION_PROJ_PRD
    else:
        version = CC7X_SB_CERT_VERSION_PROJ_PRD
    ProjDict['cert_version'] = (version << VERSION_PROJ_PRD_BIT_OFF) | (ProjDict['majorKey'] << VERSION_MAJOR_BIT_OFF) | (ProjDict['minorKey'])
