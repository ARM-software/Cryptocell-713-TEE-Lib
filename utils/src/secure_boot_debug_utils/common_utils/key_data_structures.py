...
class CertHeader:
    ...
    def __init__(self, log, LIST_OF_CONF_PARAMS, hbkId, certType, codeEncId, loadVerifyScheme, cryptoType, numOfComps, ProjDict, userAddDataFile):
        self.userAddData = bytes()
        if ProjDict['is_add_data'] == 1:
            if userAddDataFile == '':
                return None
            self.userAddData = GetDataFromBinFile(log, userAddDataFile)
            if (len(self.userAddData) != USER_ADDITIONAL_DATA_SIZE_IN_BYTES):
                return None
        ...
        self.CertVersion = ProjDict['cert_version']
    ...
    def CertHeaderLenCreate(self, log, numOfComps):
        ...
        if (len(self.userAddData) == USER_ADDITIONAL_DATA_SIZE_IN_BYTES):
            self.CertSize = self.CertSize + USER_ADDITIONAL_DATA_SIZE_IN_WORDS
        if numOfComps > 0: #this is a content certificate
            if self.ProjDict['is_iot_cert'] == 1:
                wordsPerImg = SW_REC_ADDR32_SIGNED_DATA_SIZE_IN_WORDS
            else:
                wordsPerImg = SW_REC_ADDR64_SIGNED_DATA_SIZE_IN_WORDS

            self.CertSize = self.CertSize + NONCE_SIZE_IN_WORDS + numOfComps*wordsPerImg
    ...
    def VarsToBinString(self):
        ...
        DataBinStr = byte2string(self.userAddData + DataBinStr +  DataBinStr1 + DataBinStr2 + DataBinStr3)
...
# This class represents nvcounter version field in the certificate
class CertNvcounter:
    nvcounter = 0
    InternalNumOfObjects = 2
    # Costructor
    def __init__(self, nvcounter_init=0):
        self.nvcounter = nvcounter_init
    ...
    def VarsToBinString(self):
        ...
        DataBinStr = struct.pack('<I', self.nvcounter)

        return byte2string(DataBinStr)
# End of CertNvcounter
