...
def WriteOutputToFile(log_file, HASHwordList, buffData, size, hash_file ,zeros_file):
    try:
        fobHash = open(hash_file, 'w')
    except IOError as e:
        print_and_log(log_file, "Failed opening file " + hash_file + "\n")
        sys.exit(1)

    try:
        fobZeroNum = open(zeros_file, 'w')
    except IOError as e:
        print_and_log(log_file, "Failed opening file" + zeros_file + "\n")
        sys.exit(1)
...
def main():
    ...
    WriteOutputToFile(log_file, HASHWordsList, str(hex(CalculateZeroBitCountOnData(HASHWordsList,outputWordSize))), outputWordSize,hash_file ,zeros_file)
