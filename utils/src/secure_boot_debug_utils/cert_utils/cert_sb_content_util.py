...
# The GetNvcounterVersion function returns CertNvcounter object with nvCounter word
def GetNvcounterVersion(logFile, nvcounter_word):
    CertNvcounterObj = CertNvcounter(nvcounter_word)

    return CertNvcounterObj
# End of GetSWVersion
...
def CreateCertUtility(sysArgsList):
    try:
        ...
        ArgsDict, config = content_cert_config_file_parser(config_fname, log_file, ProjDict)
        if ArgsDict == None:
            print_and_log(log_file, "Exit with illegal variable type - please check log!")
            log_file.close()
            exit(1)
        ...
        RecList = ImageFileAnalyzer(log_file, ArgsDict['images_table'], ArgsDict['load_verify_scheme'], ArgsDict['aes_ce_id'], DLLHandle, ArgsDict['aes_enc_key'], keyNonce.randStr, ArgsDict['crypto_type'], ProjDict)

        print_and_log(log_file, "\n Prepare Certificate header ")
        # Create the certificate header and add to list -> header includes
        CertDataList.append(CertHeader(log_file, LIST_OF_CONF_PARAMS, ArgsDict['hbk_id'], CERT_TYPE_CONTENT, ArgsDict['aes_ce_id'], ArgsDict['load_verify_scheme'], ArgsDict['crypto_type'], len(RecList), ProjDict, ArgsDict['add_data']))
        ...
        # Add revocation word to list
        CertDataList.append(GetNvcounterVersion(log_file, ArgsDict['nvcounter_word']))
...
if __name__ == "__main__":
    ...
    ProjDict = parseConfFile(LIST_OF_CONF_PARAMS)
